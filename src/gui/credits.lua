...
function GUI:CreateCreditsFrame(parent)
    ...
    F.CreateFS(people, C.Assets.Fonts.Heavy, 18, outline, L['People'], 'CLASS', outline or 'THICK', { 'TOP', 0, -10 })

    GUI:CreateGradientLine(people, 120, -60, -34, 60, -34)

    F.CreateFS(people, C.Assets.Fonts.Bold, 16, outline, datas.people, { 250 / 255, 206 / 255, 110 / 255 }, outline or 'THICK', { 'TOP', 0, -50 })

    local addons = CreateFrame('Frame', nil, parent)
    addons:SetSize(400, 90)
    addons:SetPoint('TOP', people, 'BOTTOM')

    F.CreateFS(addons, C.Assets.Fonts.Heavy, 18, outline, L['AddOns'], 'CLASS', outline or 'THICK', { 'TOP', 0, -10 })

    GUI:CreateGradientLine(addons, 120, -60, -34, 60, -34)

    F.CreateFS(addons, C.Assets.Fonts.Bold, 16, outline, datas.addons, { 247 / 255, 189 / 255, 94 / 255 }, outline or 'THICK', { 'TOP', 0, -50 })

    local libraries = CreateFrame('Frame', nil, parent)
    libraries:SetSize(400, 150)
    libraries:SetPoint('TOP', addons, 'BOTTOM')

    F.CreateFS(libraries, C.Assets.Fonts.Heavy, 18, outline, L['Libraries'], 'CLASS', outline or 'THICK', { 'TOP', 0, -10 })

    GUI:CreateGradientLine(libraries, 120, -60, -34, 60, -34)

    F.CreateFS(libraries, C.Assets.Fonts.Bold, 16, outline, datas.libraries, { 243 / 255, 172 / 255, 80 / 255 }, outline or 'THICK', { 'TOP', 0, -50 })
end
