...
local mask_mine_friendly_player = bit_bor(COMBATLOG_OBJECT_AFFILIATION_MASK,
                                          COMBATLOG_OBJECT_REACTION_MASK,
                                          COMBATLOG_OBJECT_CONTROL_MASK)
local flag_mine_friendly_player = bit_bor(COMBATLOG_OBJECT_AFFILIATION_MINE,
                                          COMBATLOG_OBJECT_REACTION_FRIENDLY,
                                          COMBATLOG_OBJECT_CONTROL_PLAYER)
local flag_pet_guardian = bit_bor(COMBATLOG_OBJECT_TYPE_PET, COMBATLOG_OBJECT_TYPE_GUARDIAN)
...
local function createCTFrame(frameName, spacing, maxLines, fadeDuration, timeVisible, justify,
                             width, height)
...
local function setMerge() dmgFunc = C.DB.combat.fct_merge and dmgMerge or dmgString end

local function vehicleChanged(_, _, unit, _, _, _, guid)
    if unit == 'player' then eventFrame.vehicleGUID = guid end
end
...
function COMBAT:FloatingCombatText()
    ...
    if C.DB.combat.fct_in then
        F.Mover(inFrame, L['Combat Text Incoming'], 'FCTInFrame',
                {'RIGHT', _G.UIParent, 'CENTER', -300, 0}, inFrame:GetWidth(), inFrame:GetHeight())
    end

    if C.DB.combat.fct_out then
        F.Mover(outFrame, L['Combat Text Outgoing'], 'FCTOutFrame',
                {'LEFT', _G.UIParent, 'CENTER', 300, 140}, outFrame:GetWidth(), outFrame:GetHeight())
    end

    setMerge()

    eventFrame:SetScript('OnEvent', function(self)
        local _, Event, _, sourceGUID, _, sourceFlags, _, destGUID, _, _, _, arg1, arg2, arg3, arg4,
              arg5, arg6, arg7, _, _, arg10 = CombatLogGetCurrentEventInfo()
        local vehicleGUID, playerGUID = self.vehicleGUID, self.playerGUID
        local fromMe = sourceGUID == playerGUID
        local fromPet = bit_band(sourceFlags, mask_mine_friendly_player) ==
                            flag_mine_friendly_player and
                            bit_band(sourceFlags, COMBATLOG_OBJECT_TYPE_PET) > 0
        local fromGuardian = bit_band(sourceFlags, mask_mine_friendly_player) ==
                                 flag_mine_friendly_player and
                                 bit_band(sourceFlags, COMBATLOG_OBJECT_TYPE_GUARDIAN) > 0
        local fromMine =
            fromMe or (C.DB.combat.fct_pet and (fromPet or fromGuardian)) or sourceGUID ==
                vehicleGUID
        ...
        elseif (Event == 'SPELL_DAMAGE' or Event == 'RANGE_DAMAGE') or
            (C.DB.combat.fct_periodic and Event == 'SPELL_PERIODIC_DAMAGE') then
        ...
        elseif (Event == 'SPELL_MISSED' or Event == 'RANGE_MISSED') then
            ...
            elseif fromMe or (C.DB.combat.fct_pet and fromPet) or sourceGUID == vehicleGUID then
                missString(false, arg1, arg4, arg6)
            end

        elseif Event == 'SPELL_HEAL' or
            (C.DB.combat.fct_periodic and Event == 'SPELL_PERIODIC_HEAL') then
