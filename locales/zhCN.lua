-- #TODO Finalize Chinese translation
...
do
    ...
    L['APPEARANCE_UI_SCALE_TIP'] =
        '|n调整界面的整体缩放|n推荐设置|n1080P = 1|n1440P = 1.4|n2160P = 2'
end
...
do
    ...
    L['INVENTORY_MARK_JUNK_ENABLED'] =
        '|n点击将可售出的物品归类为垃圾。|n当你开启自动出售垃圾时，这些物品也将被一同售出。|n这个列表是账号共享的，同时也不会跟随你的设置导出。|n按住CTRL+ALT并点击此按钮，可以清空这个列表。'
    L['INVENTORY_QUICK_SPLIT'] = '快速拆分'
    L['INVENTORY_SPLIT_COUNT'] = '拆分个数'
    L['INVENTORY_SPLIT_MODE_ENABLED'] =
        '|n点击拆分背包的堆叠物品，可在左侧输入框调整每次点击的拆分个数。'
    ...
    L['INVENTORY_AUTO_DEPOSIT'] =
        '|n左键点击存放材料，右键点击切换存放模式。|n当按钮高亮时，每当打开银行，将自动存放背包中的材料。'
...
do
    ...
    L['UNITFRAME_CLICK_CAST_DESC'] =
        'Ctrl/Alt/Shift + 任意鼠标按键点击想绑定的技能|n对小队或团队框体使用绑定的快捷键就能直接施放技能'
...
do
    ...
    L['NAMEPLATE_ENABLE_NAMEPLATE_TIP'] =
        '|n如果想要使用其他的姓名板插件，请先禁用该模块。'
    ...
    L['NAMEPLATE_DPS_REVERT_THREAT_TIP'] =
        '|n非坦克专精使用反转的坦克模式染色逻辑。'
    ...
    L['NAMEPLATE_CUSTOM_UNIT_LIST_TIP'] =
        '|n输入名字，以空格隔开，输入完毕按下回车键保存。'
    ...
    L['NAMEPLATE_INTERRUPT_NAME_TIP'] =
        '|n在姓名板上显示成功打断此次读条的队友名字。'

    L['NAMEPLATE_PLATE_AURAS'] = '显示光环'
    L['NAMEPLATE_PLATE_AURAS_TIP'] =
        '|n默认显示所有你施放的减益光环，可以通过黑/白名单来调整你想要显示或忽略的光环。'
    L['NAMEPLATE_AURA_SIZE'] = '光环大小'
    L['NAMEPLATE_AURA_NUMBER'] = '光环最大数量'
    L['NAMEPLATE_AURA_WHITE_LIST'] = '白名单'
    L['NAMEPLATE_AURA_BLACK_LIST'] = '黑名单'
    L['NAMEPLATE_AURA_WHITE_LIST_TIP'] =
        '|n填入想要显示的法术ID然后点击添加按钮。'
    L['NAMEPLATE_AURA_BLACK_LIST_TIP'] =
        '|n填入想要忽略的法术ID然后点击添加按钮。'
...
do
    ...
    L['INSTALL_BODY_WELCOME'] =
        '欢迎使用 |cffe9c55dFreeUI|r ！|n|n在开始使用前需要调整一些设定来更好的搭配 |cffe9c55dFreeUI|r 工作。|n|n点击安装按钮将进入安装步骤。'
    L['INSTALL_HEADER_BASIC'] = '基础设置'
    L['INSTALL_BODY_BASIC'] =
        '这些安装步骤将为 |cffe9c55dFreeUI|r 调整各类合适的设定。|n|n第一步将会调整一些 |cffe9c55dCVars|r 设定。|n|n点击下方的继续按钮将应用设定，或者点击跳过按钮如果你想跳过这些设定。'
    ...
    L['INSTALL_BODY_ADDON'] =
        '这个步骤将会调整 |cffe9c55dDBM|r 和 |cffe9c55dSkada|r 的设定以使其配合 |cffe9c55dFreeUI|r 的界面风格与布局。'
    L['INSTALL_HEADER_COMPLETE'] = '安装成功！'
    L['INSTALL_BODY_COMPLETE'] =
        '安装已经成功完成。|n|n请点击下方完成按钮重载界面。|n|n记住在游戏中你可以通过输入 |cffe9c55d/free|r 来获取详细的帮助或是直接输入 |cffe9c55d/free config|r 来打开控制面板更改各类设定。'
