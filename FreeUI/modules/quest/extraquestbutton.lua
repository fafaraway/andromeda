...
function ExtraQuestButton:PLAYER_LOGIN()
	...
	if not self:GetPoint() then
		if _G.FreeUI_ActionBarExtra then
			...
			F.Mover(self, L.GUI.MOVER.QUEST_BUTTON, 'QuestButton', {'BOTTOM', UIParent, 'BOTTOM', 250, 100})
		end
	end
...
ExtraQuestButton:SetScript('OnEnter', function(self)
	if not self.itemLink then
		if C.isDeveloper then print('ExtraQuestButton: invalid item link.') end
		return
	end
...
function ExtraQuestButton:SetItem(itemLink, texture)
	...
	if self.itemID then
		...
		F:GetModule('ACTIONBAR').UpdateHotKey(self)
