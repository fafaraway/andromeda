	...
	for bag = 0, 4 do
		for slot = 1, GetContainerNumSlots(bag) do
			...
			if link then
				...
				if (quality == 0 or FreeUIGlobalConfig['custom_junk_list'][itemID]) and price > 0 and not cache['b'..bag..'s'..slot] then
...
local function updateSelling(event, ...)
	if not FreeUIGlobalConfig['auto_sell_junk'] then return end
...
function autoRepair(override)
	...
	if canRepair and repairAllCost > 0 and FreeUIGlobalConfig['auto_repair'] then
...
local function merchantShow()
	if IsControlKeyDown() or not FreeUIGlobalConfig['auto_repair'] or not CanMerchantRepair() then return end
	autoRepair()
