...
function UNITFRAME:SpawnPlayer()
	...
	F.Mover(player, L['UNITFRAME_MOVER_PLAYER'], 'PlayerFrame',
			{'BOTTOM', _G.UIParent, 'BOTTOM', 0, 220}, player:GetWidth(),
			player:GetHeight())
	...
	if C.DB.actionbar.actionbar then
		...
		FreeUI_LeaveVehicleButton:SetSize(player:GetHeight() + 4,
										  player:GetHeight() + 4)
	end
end
...
function UNITFRAME:SpawnPet()
	...
	F.Mover(pet, L['UNITFRAME_MOVER_PET'], 'PetFrame',
...
function UNITFRAME:SpawnTarget()
	...
	F.Mover(target, L['UNITFRAME_MOVER_TARGET'], 'TargetFrame',
			{'BOTTOM', _G.UIParent, 'BOTTOM', 170, 300}, target:GetWidth(),
			target:GetHeight())
end
...
function UNITFRAME:SpawnTargetTarget()
	...
	F.Mover(targettarget, L['UNITFRAME_MOVER_TARGETTARGET'],
			'TargetTargetFrame', {'LEFT', 'oUF_Target', 'RIGHT', 6, 0},
			targettarget:GetWidth(), targettarget:GetHeight())
end
...
function UNITFRAME:SpawnFocus()
	...
	F.Mover(focus, L['UNITFRAME_MOVER_FOCUS'], 'FocusFrame',
			{'BOTTOM', _G.UIParent, 'BOTTOM', -200, 220}, focus:GetWidth(),
			focus:GetHeight())
end
...
function UNITFRAME:SpawnBoss()
	...
	for i = 1, _G.MAX_BOSS_FRAMES do
		...
		if i == 1 then
			boss[i].mover = F.Mover(boss[i], L['UNITFRAME_MOVER_BOSS'],
									'BossFrame',
									{'LEFT', 'oUF_Target', 'RIGHT', 150, 120},
...
function UNITFRAME:SpawnArena()
	...
	for i = 1, 5 do
		...
		if i == 1 then
			arena[i].mover = F.Mover(arena[i], L['UNITFRAME_MOVER_ARENA'],
									 'ArenaFrame', {
...
function UNITFRAME:SpawnParty()
	...
	local groupingOrder = partyHorizon and 'TANK,HEALER,DAMAGER,NONE' or
							  'NONE,DAMAGER,HEALER,TANK'
	local moverWidth = partyHorizon and (partyWidth * 5 + partyGap * 4) or
						   partyWidth
	local moverHeight = partyHorizon and partyHeight or
							(partyHeight * 5 + partyGap * 4)
	...
	partyMover = F.Mover(party, L['UNITFRAME_MOVER_PARTY'], 'PartyFrame',
...
function UNITFRAME:SpawnRaid()
	...
	local raidWidth, raidHeight = C.DB.unitframe.raid_width,
								  C.DB.unitframe.raid_height
	...
	for i = 1, numGroups do
		...
		if i == 1 then
			if raidHorizon then
				raidMover = F.Mover(groups[i], L['UNITFRAME_MOVER_RAID'],
									'RaidFrame', {
