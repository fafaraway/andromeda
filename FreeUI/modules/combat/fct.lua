...
eventFrame:SetScript('OnEvent', function(self)
	...
	local fromMine = fromMe or (cfg.fct_pet and band(sourceFlags, mask_mine_friendly_player) == flag_mine_friendly_player and band(sourceFlags, flag_pet_guardian) > 0) or sourceGUID == vehicleGUID
	...
	elseif (Event == 'SPELL_DAMAGE' or Event == 'RANGE_DAMAGE') or (cfg.fct_periodic and Event == 'SPELL_PERIODIC_DAMAGE') then
	...
	elseif Event == 'SPELL_HEAL' or (cfg.fct_periodic and Event == 'SPELL_PERIODIC_HEAL') then
...
local function setMerge()
	dmgFunc = cfg.fct_merge and dmgMerge or dmgString
end


function COMBAT:FloatingCombatText()
	if cfg.fct_incoming then
		local inMover = F.Mover(inFrame, L['MOVER_COMBATTEXT_INCOMING'], 'CombatText_In', {'CENTER', UIParent, 'CENTER', -300, 0}, 200, 150)
		inFrame:SetPoint('TOP', inMover)
	end

	if cfg.fct_outgoing then
		local outMover = F.Mover(outFrame, L['MOVER_COMBATTEXT_OUTGOING'], 'CombatText_Out', {'CENTER', UIParent, 'CENTER', 300, 140}, 200, 150)
		outFrame:SetPoint('TOP', outMover)
	end

	local frames = {inFrame, outFrame}
	for _, f in pairs(frames) do
		F.SetFS(f, C.Assets.Fonts.Number, 18)
...
COMBAT:RegisterCombat("FloatingCombatText", COMBAT.FloatingCombatText)