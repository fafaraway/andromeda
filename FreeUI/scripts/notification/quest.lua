...
local function acceptText(link, daily)
	if daily then
		return format('%s [%s]%s', L['NOTIFICATION_ACCEPT_QUEST'], DAILY, link)
	else
		return format('%s %s', L['NOTIFICATION_ACCEPT_QUEST'], link)
	end
end
...
function NOTIFICATION:FindQuestProgress(_, msg)
	if not C.notification.questProgress then return end
	if C.notification.onlyCompleteRing then return end
...
function NOTIFICATION:QuestNotifier()
	if C.notification.questNotifier then
