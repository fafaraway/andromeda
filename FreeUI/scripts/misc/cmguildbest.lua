...
function MISC.GuildBest_OnLoad(event, addon)
	if addon == 'Blizzard_ChallengesUI' then
		...
		F:UnregisterEvent(event, MISC.GuildBest_OnLoad)
	end
end
...
function MISC:CMGuildBest()
	F:RegisterEvent('ADDON_LOADED', self.GuildBest_OnLoad)

	self:KeystoneInfo_Update()
	F:RegisterEvent('BAG_UPDATE', self.KeystoneInfo_Update)
end