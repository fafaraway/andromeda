...
function module:PetBattleUI()
	...
	for index, unit in pairs(units) do
		...
		unit.healthBg = F.CreateBG(unit.ActualHealthBar)
	...
	F.CreateFSA(skipButton, 14, PET_BATTLE_PASS, false, "BOTTOM", 1, 2)
	...
	turnTimer.bg = F.CreateBG(turnTimer, 0)
	...
	for i = 1, NUM_BATTLE_PETS_IN_BATTLE do
		...
		unit.ActualHealthBar:SetTexture(C.media.texture)
		local bg = F.CreateBG(unit.ActualHealthBar)
	...
	F.CreateGF(lineLeft, 180, 1, "Horizontal", r, g, b, 0, .7)
	...
	F.CreateGF(lineRight, 180, 1, "Horizontal", r, g, b, .7, 0)
