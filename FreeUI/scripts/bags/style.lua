...
local itemSlotSize = C.bags.itemSlotSize
------------------------------------------
...
local function SellJunk()
	...
	if Profit > 0 then
		print(C.InfoColor..L["SellJunk"]..": ".."|cffffffff"..GetMoneyString(Profit).."|r".."  |cffd366ae("..SoldCount..")|r")
	end
end
...
function MyContainer:OnCreate(name, settings)
	...
	if (tBank or tBankBags) then
		self.Columns = (usedSlotsBank > C.bags.bankLargeItemCount) and C.bags.bankColumnsLarge or C.bags.bankColumnsSmall
	elseif (tReagent) then
		self.Columns = (usedSlotsReagent > C.bags.bankLargeItemCount) and C.bags.bankColumnsLarge or C.bags.bankColumnsSmall
	else
		self.Columns = (usedSlotsBag > C.bags.bagColumnsLarge) and C.bags.bagColumnsLarge or C.bags.bagColumnsSmall
	end
