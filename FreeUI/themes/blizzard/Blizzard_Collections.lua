...
C.Themes["Blizzard_Collections"] = function()
	...
	for _, scrollFrame in pairs(scrollFrames) do
		for i = 1, #scrollFrame do
			...
			if bu.DragButton then
				bu.DragButton.ActiveTexture:SetTexture(C.Assets.Textures.check)
	...
	for i = 1, 3 do
		...
		for j = 1, 3 do
			...
			spell.selected:SetTexture(C.Assets.Textures.check)
			spell:GetRegions():Hide()

			spell.FlyoutArrow:SetTexture(C.Assets.arrowDown)
	...
	for i = 1, 2 do
		...
		bu:SetCheckedTexture(C.Assets.Textures.check)
		bu:SetPushedTexture("")
	...
	hooksecurefunc(HeirloomsJournal, "LayoutCurrentPage", function()
		for i = 1, #HeirloomsJournal.heirloomHeaderFrames do
			...
			if not header.styled then
				...
				header.text:SetFont(C.HeaderFont, 16, "OUTLINE")
