...
tinsert(C.BlizzThemes, function()
	...
	for index, unit in pairs(units) do
		...
		unit.ActualHealthBar:SetTexture(C.Assets.Textures.statusbar)
		...
		unit.Iconbg = F.CreateBDFrame(unit.Icon, nil, true)
	...
	hooksecurefunc("PetBattleFrame_UpdateActionBarLayout", function(self)
		...
		for i = 1, 6 do
			...
			bu:SetNormalTexture("")
			bu:GetPushedTexture():SetTexture(C.Assets.Textures.check)
	...
	skipButton:SetPushedTexture(C.Assets.Textures.check)

	local xpbar = PetBattleFrameXPBar
	F.StripTextures(xpbar)
	xpbar:SetParent(bar)
	xpbar:SetWidth(bar:GetWidth())
	xpbar:SetStatusBarTexture(C.Assets.Textures.statusbar)
	F.CreateBDFrame(xpbar, nil, true)
	...
	turnTimer.bg = F.CreateBDFrame(turnTimer, nil, true)
