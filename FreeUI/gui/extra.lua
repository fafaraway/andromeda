...
local function hideExtraGUIs()
	for _, frame in pairs(extraGUIs) do frame:Hide() end
end
...
local function createExtraGUI(parent, name, title, bgFrame)
	...
	if title then
		F.CreateFS(frame, C.Assets.Fonts.Regular, 14, nil, title, 'YELLOW',
				   true, 'TOPLEFT', 10, -25)
	end
...
local function createOptionCheck(parent, offset, text)
	...
	F.CreateFS(box, C.Assets.Fonts.Regular, 12, nil, text, nil, true, 'LEFT',
			   22, 0)
	return box
end
...
local function createLabel(parent, text, tip)
	local label = F.CreateFS(parent, C.Assets.Fonts.Regular, 12, nil, text,
							 'YELLOW', true, 'CENTER', 0, 25)
...
local function clearEdit(options)
	for i = 1, #options do GUI:ClearEdit(options[i]) end
end
...
function GUI:CreateScroll(parent, width, height, text)
	...
	if text then
		F.CreateFS(scroll, C.Assets.Fonts.Regular, 12, 'OUTLINE', text, nil,
				   true, 'TOPLEFT', 5, 20)
	end
...
local function createOptionTitle(parent, title, offset)
	F.CreateFS(parent, C.Assets.Fonts.Regular, 14, nil, title, 'YELLOW', true,
			   'TOP', 0, offset)
...
local function createOptionSlider(parent, title, minV, maxV, step, defaultV, x,
								  y, value, func)
...
function GUI:SetupInventoryFilter(parent)
	...
	for _, value in ipairs(filterOptions) do
		local box = createOptionCheck(scroll, offset,
									  L.GUI.INVENTORY[strupper(value)])
...
function GUI:SetupActionBarFader(parent)
	...
	local function Update() F.ACTIONBAR:UpdateActionBarFade() end
	...
	local function createOptionSliders(parent, title, minV, maxV, step,
									   defaultV, x, y, value, func)
		local slider =
			F.CreateSlider(parent, title, minV, maxV, step, x, y, 180)
	...
	local offset = 20
	for _, value in ipairs(filterOptions) do
		local box = createOptionCheck(scroll.child, offset,
									  L.GUI.ACTIONBAR[strupper(value)])
	...
	createOptionSliders(scroll.child, L.GUI.ACTIONBAR.FADE_OUT_ALPHA, 0, 1, .1,
						faderValues[1], 20, -offset - 20, 'fade_out_alpha',
						Update)
	createOptionSliders(scroll.child, L.GUI.ACTIONBAR.FADE_IN_ALPHA, 0, 1, .1,
						faderValues[2], 20, -offset - 100, 'fade_in_alpha',
						Update)
	createOptionSliders(scroll.child, L.GUI.ACTIONBAR.FADE_OUT_DURATION, 0, 1,
						.1, faderValues[3], 20, -offset - 180,
						'fade_out_duration', Update)
	createOptionSliders(scroll.child, L.GUI.ACTIONBAR.FADE_IN_DURATION, 0, 1,
						.1, faderValues[4], 20, -offset - 260,
						'fade_in_duration', Update)
end
...
function GUI:NamePlateAuraFilter(parent)
	...
	local function createBar(parent, index, spellID)
		...
		local spellName = F.CreateFS(bar, C.Assets.Fonts.Regular, 12, nil, name,
									 nil, true, 'LEFT', 30, 0)
	...
	local function addClick(parent, index)
		...
		if not spellID or not GetSpellInfo(spellID) then
			UIErrorsFrame:AddMessage(C.RedColor ..
										 L.GUI.NAMEPLATE.AURA_INCORRECT_ID)
			return
		end
		if FREE_ADB['nameplate_aura_filter_list'][index][spellID] then
			UIErrorsFrame:AddMessage(C.RedColor ..
										 L.GUI.NAMEPLATE.AURA_EXISTING_ID)
			return
		end
	...
	for index, value in ipairs(frameData) do
		F.CreateFS(panel, C.Assets.Fonts.Regular, 14, nil, value.text, 'YELLOW',
				   true, 'TOPLEFT', 20, value.offset)
...
function GUI:SetupUnitFrameSize(parent)
	...
	local panel = createExtraGUI(parent, guiName,
								 L.GUI.UNITFRAME.UNITFRAME_SIZE_SETTING_HEADER)
	local scroll = GUI:CreateScroll(panel, 220, 540)
	...
	local function UpdateSize(self, unit) end

	local function createOptionGroup(parent, title, offset, value, func)
		createOptionTitle(parent, title, offset)
		createOptionSlider(parent, L.GUI.UNITFRAME.SET_WIDTH,
						   sliderRange[value][1], sliderRange[value][2], 1,
						   defaultValue[value][1], 20, offset - 60,
						   value .. '_width', func)
		createOptionSlider(parent, L.GUI.UNITFRAME.SET_HEIGHT, 4, 20, 1,
						   defaultValue[value][2], 20, offset - 130,
						   value .. '_height', func)
	end

	local function createPowerOptionGroup(parent, title, offset, value, func)
		createOptionTitle(parent, title, offset)
		createOptionSlider(parent, L.GUI.UNITFRAME.SET_POWER_HEIGHT, 1, 10, 1,
						   1, 20, offset - 60, 'power_bar_height', func)
		createOptionSlider(parent, L.GUI.UNITFRAME.SET_ALT_POWER_HEIGHT, 1, 10,
						   1, 2, 20, offset - 130, 'alt_power_height', func)
	end

	createOptionGroup(scroll.child, L.GUI.UNITFRAME.CAT_PLAYER, -10, 'player',
					  UpdateSize)
	createOptionGroup(scroll.child, L.GUI.UNITFRAME.CAT_TARGET, -210, 'target',
					  UpdateSize)
	createOptionGroup(scroll.child, L.GUI.UNITFRAME.CAT_FOCUS, -410, 'focus',
					  UpdateSize)
	createOptionGroup(scroll.child, L.GUI.UNITFRAME.CAT_PET, -610, 'pet',
					  UpdateSize)
	createOptionGroup(scroll.child, L.GUI.UNITFRAME.CAT_BOSS, -810, 'boss',
					  UpdateSize)
	createOptionGroup(scroll.child, L.GUI.UNITFRAME.CAT_ARENA, -1010, 'arena',
					  UpdateSize)
	createPowerOptionGroup(scroll.child, L.GUI.UNITFRAME.CAT_POWER, -1210, nil,
						   UpdateSize)
end
...
function GUI:SetupGroupFrameSize(parent)
	...
	local panel = createExtraGUI(parent, guiName,
								 L.GUI.UNITFRAME.GROUPFRAME_SIZE_SETTING_HEADER)
	local scroll = GUI:CreateScroll(panel, 220, 540)
	...
	local function createOptionGroup(parent, title, offset, value, func)
		...
		createOptionSlider(parent, L.GUI.UNITFRAME.SET_WIDTH,
						   ...
						   defaultValue[value][1], 20, offset - 60,
						   value .. '_width', func)
		createOptionSlider(parent, L.GUI.UNITFRAME.SET_HEIGHT,
						   sliderRange[value][1], sliderRange[value][2], 1,
						   defaultValue[value][2], 20, offset - 130,
						   value .. '_height', func)
		createOptionSlider(parent, L.GUI.UNITFRAME.SET_GAP, 5, 10, 1,
						   defaultValue[value][3], 20, offset - 200,
						   value .. '_gap', func)
	end

	createOptionGroup(scroll.child, L.GUI.GROUPFRAME.CAT_PARTY, -10, 'party',
					  UpdateSize)
	createOptionGroup(scroll.child, L.GUI.GROUPFRAME.CAT_RAID, -280, 'raid',
					  UpdateSize)
...
function GUI:SetupUnitFrameFader(parent)
	...
	local panel = createExtraGUI(parent, guiName,
								 L.GUI.UNITFRAME.FADER_SETTING_HEADER)
	local scroll = GUI:CreateScroll(panel, 220, 540)
	...
	for _, value in ipairs(filterOptions) do
		local box = createOptionCheck(scroll.child, offset,
									  L.GUI.UNITFRAME[strupper(value)])
	...
	createOptionSlider(scroll.child, L.GUI.UNITFRAME.FADE_OUT_ALPHA, 0, 1, .1,
					   faderValues[1], 20, -offset - 20, 'fade_out_alpha',
					   Update)
	createOptionSlider(scroll.child, L.GUI.UNITFRAME.FADE_IN_ALPHA, 0, 1, .1,
					   faderValues[2], 20, -offset - 100, 'fade_in_alpha',
					   Update)
	createOptionSlider(scroll.child, L.GUI.UNITFRAME.FADE_OUT_DURATION, 0, 1,
					   .1, faderValues[3], 20, -offset - 180,
					   'fade_out_duration', Update)
	createOptionSlider(scroll.child, L.GUI.UNITFRAME.FADE_IN_DURATION, 0, 1, .1,
					   faderValues[4], 20, -offset - 260, 'fade_in_duration',
					   Update)
end
...
function GUI:SetupCastbar(parent)
	...
	local panel = createExtraGUI(parent, guiName,
								 L.GUI.UNITFRAME.CASTBAR_SETTING_HEADER)
	local scroll = GUI:CreateScroll(panel, 220, 540)

	createOptionTitle(scroll.child, L.GUI.UNITFRAME.CASTBAR_COLORS, -10)
	createOptionSwatch(scroll.child, L.GUI.UNITFRAME.CASTING_COLOR,
					   C.DB.unitframe.casting_color, 40, -40)
	createOptionSwatch(scroll.child,
					   L.GUI.UNITFRAME.CASTING_UNINTERRUPTIBLE_COLOR,
	...
	local function createOptionGroup(parent, title, offset, value, func)
		...
		createOptionSlider(parent, L.GUI.UNITFRAME.CASTBAR_WIDTH, 100, 400, 1,
						   defaultValue[value][1], 20, offset - 60,
						   'castbar_' .. value .. '_width', func)
		createOptionSlider(parent, L.GUI.UNITFRAME.CASTBAR_HEIGHT, 6, 30, 1,
						   defaultValue[value][2], 20, offset - 130,
						   'castbar_' .. value .. '_height', func)
	end

	local function updateFocusCastbar() end

	createOptionGroup(scroll.child, L.GUI.UNITFRAME.CASTBAR_FOCUS, -180,
					  'focus', updateFocusCastbar)
end
...
function GUI:SetupPartySpellCooldown(parent)
	...
	local panel = createExtraGUI(parent, guiName,
								 L.GUI.GROUPFRAME.PARTY_SPELL_SETTING_HEADER,
								 true)
	...
	local function createBar(parent, spellID, duration)
		...
		local name = F.CreateFS(bar, C.Assets.Fonts.Regular, 12, nil, spellName,
								nil, true, 'LEFT', 30, 0)
		name:SetWidth(120)
		name:SetJustifyH('LEFT')

		local timer = F.CreateFS(bar, C.Assets.Fonts.Regular, 12, nil, duration,
								 nil, true, 'RIGHT', -30, 0)
		timer:SetWidth(60)
	...
	options[2] = GUI:CreateEditbox(frame, L.GUI.GROUPFRAME.SPELL_COOLDOWN, 120,
								   -30, L.GUI.GROUPFRAME.SPELL_COOLDOWN_TIP, 90,
								   24)
	...
	local function addClick(scroll, options)
		local spellID, duration = tonumber(options[1]:GetText()),
								  tonumber(options[2]:GetText())
		if not spellID or not duration then
			UIErrorsFrame:AddMessage(C.RedColor ..
										 L.GUI.GROUPFRAME.INCOMPLETE_INPUT)
			return
		end
		if not GetSpellInfo(spellID) then
			UIErrorsFrame:AddMessage(C.RedColor ..
										 L.GUI.GROUPFRAME.INCORRECT_SPELLID)
			return
		end
	...
	scroll.add:SetScript('OnClick', function() addClick(scroll, options) end)

	scroll.clear = F.CreateButton(frame, 60, 24, KEY_NUMLOCK_MAC)
	scroll.clear:SetPoint('RIGHT', scroll.add, 'LEFT', -10, 0)
	scroll.clear:SetScript('OnClick', function() clearEdit(options) end)
...
local function UpdateGroupDebuffs() F.UNITFRAME:UpdateGroupDebuffs() end
...
function GUI:SetupGroupDebuffs(parent)
	...
	local panel = createExtraGUI(parent, guiName,
								 L.GUI.GROUPFRAME.GROUP_DEBUFF_SETTING_HEADER,
								 true)
	...
	local iType = GUI:CreateDropdown(frame, L.GUI.GROUPFRAME.TYPE, 10, -30,
									 {DUNGEONS, RAID},
	...
	for i = 1, 2 do
		iType.options[i]:HookScript('OnClick', function()
			...
			for k = 1, #bars do bars[k]:Hide() end
		end)
	end
	...
	for dungeonID = 1182, 1189 do
		...
		if name then tinsert(dungeons, name) end
	end
	...
	options[1]:Hide()
	options[2] = GUI:CreateDropdown(frame, RAID, 120, -30, raids,
									L.GUI.GROUPFRAME.RAID_TIP, 90, 24)
	options[2]:Hide()
	...
	local function isAuraExisted(instName, spellID)
		...
		local savedPrio = FREE_ADB['raid_debuffs_list'][instName] and
							  FREE_ADB['raid_debuffs_list'][instName][spellID]
		if (localPrio and savedPrio and savedPrio == 0) or
			(not localPrio and not savedPrio) then return false end
		return true
	end

	local function addClick(options)
		local dungeonName, raidName, spellID, priority =
			options[1].Text:GetText(), options[2].Text:GetText(),
			tonumber(options[3]:GetText()), tonumber(options[4]:GetText())
	...
	scroll.add:SetScript('OnClick', function() addClick(options) end)
	scroll.clear = F.CreateButton(frame, 60, 24, KEY_NUMLOCK_MAC)
	scroll.clear:SetPoint('RIGHT', scroll.add, 'LEFT', -10, 0)
	scroll.clear:SetScript('OnClick', function() clearEdit(options) end)
	...
	local function createBar(index, texture)
		...
		local spellName = F.CreateFS(bar, C.Assets.Fonts.Regular, 11, nil, '',
									 nil, true, 'LEFT', 26, 0)
		...
		prioBox:HookScript('OnEscapePressed',
						   function(self) self:SetText(bar.priority) end)
		...
		F.AddTooltip(prioBox, 'ANCHOR_RIGHT',
					 L.GUI.GROUPFRAME.PRIORITY_EDITBOX_TIP, 'BLUE')
		bar.prioBox = prioBox
	...
	local function applyData(index, instName, spellID, priority)
		local name, _, texture = GetSpellInfo(spellID)
		if not bars[index] then bars[index] = createBar(index, texture) end
	...
	function setupBars(self)
		...
		if C.RaidDebuffsList[instName] then
			for spellID, priority in pairs(C.RaidDebuffsList[instName]) do
				if not (FREE_ADB['raid_debuffs_list'][instName] and
					FREE_ADB['raid_debuffs_list'][instName][spellID]) then
					index = index + 1
		...
		if FREE_ADB['raid_debuffs_list'][instName] then
			for spellID, priority in pairs(
										 FREE_ADB['raid_debuffs_list'][instName]) do
				if priority > 0 then
					index = index + 1
					applyData(index, instName, spellID, priority)
				end
		...
		for i = 1, #bars do if i > index then bars[i]:Hide() end end
